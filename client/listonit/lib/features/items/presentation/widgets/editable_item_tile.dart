...
class EditableItemTile extends ConsumerStatefulWidget {
  final Item item;
  final String listId;
  ...
  @override
  ConsumerState<EditableItemTile> createState() => _EditableItemTileState();
}

class _EditableItemTileState extends ConsumerState<EditableItemTile>
  ...
  @override
  void didUpdateWidget(EditableItemTile oldWidget) {
  ...
  void _debouncedSave({
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 500), () async {
      final success = await ref.read(itemsProvider(widget.listId).notifier).updateItem(
  ...
  Future<void> _toggleItem() async {
    ...
    await ref.read(itemsProvider(widget.listId).notifier).toggleItem(widget.item.id);
  }
        ...
        child: AnimatedContainer(
          ...
          child: Column(
            ...
            children: [
              // Main tile
              ListTile(
                leading: _buildAnimatedCheckbox(),
                title: _isExpanded
                    ? _buildNameField()
                    : GestureDetector(
                ...
                onTap: widget.item.isChecked ? () => _toggleItem() : null,
              ),

              // Expanded edit form
              if (_isExpanded) _buildExpandedForm(context),
            ],
          ),
        ),
      ),
